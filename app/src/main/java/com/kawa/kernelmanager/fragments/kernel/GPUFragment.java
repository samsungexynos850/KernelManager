...
import com.kawa.kernelmanager.utils.kernel.gpu.AdrenoIdler;
import com.kawa.kernelmanager.utils.kernel.gpu.AdrenoBoost;
import com.kawa.kernelmanager.utils.kernel.gpu.GPUFreq;
import com.kawa.kernelmanager.utils.kernel.gpu.GPUFreqExynos;
import com.kawa.kernelmanager.utils.kernel.gpu.GPUFreqTmu;
import com.kawa.kernelmanager.utils.kernel.gpu.SimpleGPU;
...
import com.kawa.kernelmanager.views.recyclerview.XYGraphView;
...
public class GPUFragment extends RecyclerViewFragment {
    ...
    private void freqInit(List<RecyclerViewItem> items) {
        ...
        if (mGPUFreqExynos.hasCurVolt()) {
            mCurVolt = new XYGraphView();
            mCurVolt.setTitle(getString(R.string.gpu_volt));
            freqCard.addItem(mCurVolt);
        }
...
//            }
        ...
        if (mCurVolt != null) {
            int volt = mGPUFreqExynos.getCurVolt();
            float maxVolt;
            try {
                maxVolt = Utils.strToFloat(mGPUFreqExynos.getStockVoltages().get(0));
            } catch (Exception e){
